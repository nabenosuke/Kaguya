...
public class Boss : MonoBehaviour
{
    ...
    private float hpGaugeSpeed = 0.25f;
    ...
    void FixedUpdate()
    {
        ...
        anim.SetFloat("XSpeed", rb.velocity.x * transform.localScale.x);
    }
    ...
    private void Move()
    {
        if (Vector2.Distance(transform.position, nextPosition) > 0.1f)
        {
            ...
            rb.velocity = new Vector2(nextPosition.x - transform.position.x, nextPosition.y - transform.position.y).normalized * speed;
